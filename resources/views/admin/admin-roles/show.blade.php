...
@section('title', 'View Admin Role')
@section('page-title', 'View Admin Role')

@section('admin-content')
    <div class="w-full px-4 md:px-8 lg:px-16 space-y-8">
        <!-- Page Header -->
        <x-ui.page-header title="View Admin Role" description="Detailed information about this admin role." />
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 w-full max-w-full">
            ...
            <div class="flex flex-wrap justify-end gap-2 mb-6 border-b border-gray-200 dark:border-gray-700 pb-4">
                <a href="{{ route('admin.admin-roles.edit', $adminRole->id) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-gray-900 dark:text-white font-medium">{{ $adminRole->id }}</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Name</label>
                    <p class="text-gray-900 dark:text-white text-lg font-semibold">{{ $adminRole->name }}</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Slug</label>
                    <span
                        class="inline-block px-3 py-1 bg-gray-100 dark:bg-gray-700 rounded text-gray-900 dark:text-white text-sm font-medium">{{ $adminRole->slug }}</span>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Description</label>
                    <p class="text-gray-900 dark:text-white">{{ $adminRole->description ?? 'N/A' }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 border-t border-gray-200 dark:border-gray-700 pt-4">
                <div>
                    ...
                    <p class="text-gray-900 dark:text-white">{{ $adminRole->created_at->format('M d, Y h:i A') }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">Updated At</label>
                    <p class="text-gray-900 dark:text-white">{{ $adminRole->updated_at->format('M d, Y h:i A') }}</p>
                </div>
            </div>
        </div>
    </div>

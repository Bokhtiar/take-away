...
<script>
    ...
    /** Match menu URL without treating /admin/orders as prefix of /admin/orders/today */
    function menuItemIsActive(menuUrl) {
        if (!menuUrl) return false;
        const path = window.location.pathname;
        if (menuUrl === '/admin/orders') {
            return path === '/admin/orders';
        }
        return path === menuUrl || path.startsWith(menuUrl + '/');
    }

    function renderMenuItem(menu, level = 0) {
        const hasChildren = menu.children && menu.children.length > 0;
        const isActive = menuItemIsActive(menu.menu_url);
